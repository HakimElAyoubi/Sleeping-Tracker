...
def _page_dashboard():
    ...
    week_debt = calculate_weekly_debt(
        [{"duration_hours": r.duration_hours} for r in recent]
    )
...
def _page_report():
    ...
    debt = calculate_weekly_debt(
        [{"duration_hours": r.duration_hours} for r in records]
    )
    ...
    if debt > 0:
        st.warning(
            f"You have a sleep debt of {debt} hours this week. Try to get more rest!"
        )
    elif debt < 0:
        st.success(
            f"Great! You slept {abs(debt)} hours more than your target this week."
        )
    else:
